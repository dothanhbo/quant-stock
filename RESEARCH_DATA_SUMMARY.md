...
- **Final entry model:** `hybrid_trend_donchian_v1__trend_context`.
- **Final exit configuration:** ATR stop `2.0`, ATR target `5.0`, maximum holding period `30` days.
...
The strongest buckets are **11–15**, **21–30**, and **31+ days**. Trades closed within ten days are negative on average. This directly supports the decision to increase the maximum holding period from 10 to 30 days.
...
The bootstrap results indicate that uncertainty in future trade quality is more important than the historical ordering of trades. These simulations use a simplified fixed-fraction model and do not reproduce portfolio concurrency, cash drag, correlated signals or regime clustering exactly.
...
1. The project found a plausible trend-following edge only after allowing winners more time and using a wider ATR target.
2. Hybrid Trend Context improves signal quality relative to pure Donchian without becoming as restrictive as the strict hybrid.
3. The edge is driven by asymmetric payoffs and long-held winners, not by a high win rate.
4. Transaction costs are material and must remain included in all future tests.
5. Monte Carlo and risk simulations support robustness, but they are approximations and do not replace live or paper-trading validation.
6. The current configuration is a research candidate, not a production-ready or investment-recommendation system.
...
1. Non-overlapping portfolio OOS validation for the selected 30 / 2.0 / 5.0 configuration.
2. Market-regime attribution: bull, bear and sideways periods.
3. Correlation-aware portfolio allocation and hard exposure limits.
4. Daily paper-trading journal using frozen strategy parameters.
5. Compare backtest assumptions with actual fills and transaction costs during paper trading.
