...
def generate_candidates_exact(kwargs: dict[str, Any], start_date: str, end_date: str) -> list[Any]:
    """
    Research adapter around the engine's candidate generator.

    We inspect the installed function signature and pass only parameters that
    the local engine actually accepts. This keeps the research script aligned
    with the repository version instead of duplicating candidate-generation
    logic.
    """
    sig = inspect.signature(generate_candidate_trades)
    params = sig.parameters

    candidate_kwargs: dict[str, Any] = {}
    for key, value in kwargs.items():
        if key in params:
            candidate_kwargs[key] = value

    # Date names used by the engine are normally start_date/end_date.
    if "start_date" in params:
        candidate_kwargs["start_date"] = start_date
    if "end_date" in params:
        candidate_kwargs["end_date"] = end_date

    result = generate_candidate_trades(**candidate_kwargs)

    # Be tolerant of engines returning (trades, ...).
