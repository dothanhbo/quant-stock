...
DEFAULT_OUTPUT_DIR = Path(
    "research_results/cross_sectional_ranking_holdout20"
)
...
def build_cases() -> list[RankingCase]:
    return [
        RankingCase("baseline__signal_score", "signal_score"),
        RankingCase("leadership__relative_strength", "relative_strength"),
        RankingCase(
            "leadership__cross_sectional_percentile",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled OOS ranking ablation. Entry, exit and sizing stay "
            "fixed; only same-day candidate ordering changes."
        )
    )
...
def build_backtest_kwargs(
    *,
    ...
    return {
        ...
        "ranking_method": case.ranking_method,
...
def main() -> None:
    ...
    for index, case in enumerate(cases, start=1):
        print("\n" + "#" * 96)
        print(f"RANKING ABLATION {index}/{len(cases)}: {case.case_id}")
        print("#" * 96)
        ...
        folds.insert(0, "ranking_method", case.ranking_method)
        folds.insert(0, "case_id", case.case_id)
        fold_frames.append(folds)
        pd.DataFrame(summary_rows).to_csv(
            output_dir / "ranking_summary_partial.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    summary_path = output_dir / "cross_sectional_ranking_summary.csv"
    folds_path = output_dir / "cross_sectional_ranking_folds.csv"
    summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    folds_df.to_csv(folds_path, index=False, encoding="utf-8-sig")

    print("\n" + "=" * 140)
    print("CROSS-SECTIONAL RANKING SUMMARY")
    print("=" * 140)
