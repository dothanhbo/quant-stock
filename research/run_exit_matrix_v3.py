...
class HybridTrailingATRExitModel(TrailingATRExitModel):
    """Research-only hybrid trail: wide trail first, then tighten after a higher R milestone.

    Example 3_to_2:
      - initial stop = 2 ATR
      - fixed target = 5 ATR
      - after +1R, trail at 3 ATR
      - after +2R, tighten to 2 ATR

    The stop is only updated after a session closes, preserving engine parity.
    """
    ...
    def __init__(
        ...
        *,
        activation_r: float,
        tighten_r: float,
        early_trailing_atr: float,
        tight_trailing_atr: float,
        **kwargs,
    ) -> None:
        if activation_r <= 0 or tighten_r <= activation_r:
            raise ValueError("Require 0 < activation_r < tighten_r")
        if early_trailing_atr <= 0 or tight_trailing_atr <= 0:
            raise ValueError("Trailing ATR multipliers must be positive")
        super().__init__(
            trailing_atr_multiplier=early_trailing_atr,
            **kwargs,
        )
        self.activation_r = float(activation_r)
        self.tighten_r = float(tighten_r)
        self.early_trailing_atr = float(early_trailing_atr)
        self.tight_trailing_atr = float(tight_trailing_atr)
    ...
    def update_levels(
        ...
        *,
        ...
        r_multiple = (
            (highest_price - float(entry_price)) / initial_risk
            if initial_risk > 0
            else 0.0
        )
        ...
        multiplier = (
            self.tight_trailing_atr
            if r_multiple >= self.tighten_r
            else self.early_trailing_atr
        )

        atr = ATRExitModel._get_atr(current_row)
        trailing_stop = highest_price - multiplier * atr
        return max(float(current_stop), trailing_stop), current_target


def build_cases() -> list[ExitPolicyCase]:
    """Structural exit matrix V3.

    Entry, sizing, regime policy, costs and execution remain frozen.
    The matrix varies only exit geometry.

    Hybrid convention:
      3_to_2 = trail 3 ATR after +1R, tighten to 2 ATR after +2R.
      4_to_2 = trail 4 ATR after +1R, tighten to 2 ATR after +2R.
    """
    return [
        # Fixed ATR controls
        ...
        ExitPolicyCase("fixed_atr_3_5", "fixed", 3.0, 5.0),
        ExitPolicyCase("fixed_atr_3_6", "fixed", 3.0, 6.0),

        # Immediate trailing
        ExitPolicyCase(
            "trailing_atr_2_0_current", "trailing", 2.0, 5.0,
            trailing_atr=2.0, is_current_production=True,
        ),
        ExitPolicyCase(
            "trailing_atr_2_5", "trailing", 2.0, 5.0,
            trailing_atr=2.5,
        ),
        ExitPolicyCase(
            "trailing_atr_3_0", "trailing", 2.0, 5.0,
            trailing_atr=3.0,
        ),
        ExitPolicyCase(
            "trailing_atr_3_5", "trailing", 2.0, 5.0,
            trailing_atr=3.5,
        ),
        ExitPolicyCase(
            "trailing_atr_4_0", "trailing", 2.0, 5.0,
            trailing_atr=4.0,
        ),

        # Delayed trailing: activation only, no tightening.
        ExitPolicyCase(
            "trailing_atr_2_5_after_1r", "delayed_trailing", 2.0, 5.0,
            trailing_atr=2.5, activation_r=1.0,
        ),
        ExitPolicyCase(
            "trailing_atr_3_0_after_1r", "delayed_trailing", 2.0, 5.0,
            trailing_atr=3.0, activation_r=1.0,
        ),
        ExitPolicyCase(
            "trailing_atr_3_5_after_1r", "delayed_trailing", 2.0, 5.0,
            trailing_atr=3.5, activation_r=1.0,
        ),

        # Hybrid: wide trail first, tighten only after +2R.
        ExitPolicyCase(
            "hybrid_3_to_2", "hybrid", 2.0, 5.0,
            trailing_atr=2.0,
        ),
        ExitPolicyCase(
            "hybrid_4_to_2", "hybrid", 2.0, 5.0,
            trailing_atr=2.0,
        ),
        ExitPolicyCase(
            "hybrid_4_to_2_5", "hybrid", 2.0, 5.0,
            trailing_atr=2.5,
        ),
    ]
...
def build_exit_model(case: ExitPolicyCase):
    ...
    if case.exit_family == "hybrid":
        ...
        return HybridTrailingATRExitModel(
            activation_r=1.0,
            tighten_r=2.0,
            early_trailing_atr=early,
            tight_trailing_atr=tight,
            stop_atr_multiplier=case.stop_atr,
            target_atr_multiplier=case.target_atr,
        )
...
def main() -> None:
    ...
    config = {
        ...
        "integrity": {
            ...
            "hybrid_is_research_only": True,
            "hybrid_definition": "wide trail after +1R, tighten after +2R",
        },
    }
    ...
    # Research-only family stability summary. This is deliberately descriptive:
    # it does not select a production policy or change the nested WFO.
    family_frame = matrix_summary.copy()
    family_frame["exit_family"] = family_frame["exit_family"].astype(str)
    family_summary = (
        family_frame.groupby("exit_family", dropna=False)
        .agg(
            policies=("case_id", "count"),
            best_wfo_return_pct=("walk_forward_return_pct", "max"),
            median_wfo_return_pct=("walk_forward_return_pct", "median"),
            best_median_test_return_pct=("median_test_return_pct", "max"),
            best_recent_3_return_pct=("recent_3_folds_return_pct", "max"),
            best_chained_dd_pct=("chained_max_drawdown_pct", "max"),
            profitable_policy_count=("research_gate_passed", "sum"),
        )
        .reset_index()
        .sort_values(
            ["best_wfo_return_pct", "best_median_test_return_pct"],
            ascending=False,
        )
    )
    family_summary.to_csv(
        output_dir / "family_stability_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )

    # Local-neighborhood sensitivity: report the spread of WFO returns within
    # each structural family. A narrow spread is generally more reassuring
    # than a single isolated peak.
    family_frame["wfo_return_pct"] = pd.to_numeric(
        family_frame["walk_forward_return_pct"], errors="coerce"
    )
    neighborhood = (
        family_frame.groupby("exit_family")["wfo_return_pct"]
        .agg(["min", "median", "max", "std", "count"])
        .reset_index()
        .rename(
            columns={
                "min": "wfo_return_min_pct",
                "median": "wfo_return_median_pct",
                "max": "wfo_return_max_pct",
                "std": "wfo_return_std_pct",
                "count": "policy_count",
            }
        )
    )
    neighborhood["wfo_range_pct"] = (
        neighborhood["wfo_return_max_pct"]
        - neighborhood["wfo_return_min_pct"]
    )
    neighborhood.to_csv(
        output_dir / "family_parameter_stability.csv",
        index=False,
        encoding="utf-8-sig",
    )
