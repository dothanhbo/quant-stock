...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fragile-scales", nargs="+", type=float, default=[0.20, 0.25, 0.30, 0.40, 0.50])
    parser.add_argument("--output", default="research_results/q70_exposure_mechanism_wfo_causal")
    return parser
...
def main() -> None:
    ...
    for fold in folds:
        print("\n" + "=" * 90)
        print(f"FOLD {fold.fold}: {fold.train_start.date()} -> {fold.test_end.date()}")
        ...
        q70_candidates = [
            t for t in test_cache.candidates if quality(t) >= args.quality_threshold
        ]

        print(f"train_candidates={len(train_cache.candidates)} test_candidates={len(test_cache.candidates)} q70_candidates={len(q70_candidates)}")
        ...
        for name, fragile_scale in policies:
            class ExposureSizer:
                ...
                @property
                def name(self):
                    return f"{self.base_sizer.name}__q70_fragile_{fragile_scale:g}"
            ...
            rows.append({
                ...
                "profit_factor": float(sum(max(0.0, num(getattr(t, "net_return_pct", 0.0))) for t in trades) / max(-sum(min(0.0, num(getattr(t, "net_return_pct", 0.0))) for t in trades), 1e-12)),
            })
            for trade in trades:
                trade_rows.append({
                    "policy": name,
    ...
    fold_df.to_csv(output / "q70_exposure_fold_summary.csv", index=False)
    trade_df.to_csv(output / "q70_exposure_trade_level.csv", index=False)

    summary_rows = []
    for policy_name, group in fold_df.groupby("policy", sort=False):
