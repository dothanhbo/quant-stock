...
def main():
    ...
    p.add_argument("--output-dir", default="research_results/state_quality_portfolio_wfo")
    ...
    # Use the same research universe as the existing current WFO unless
    # --symbols-file is added later. This script intentionally does not invent
    ...
    try:
        ...
        raise RuntimeError(
            "Could not import HOLDOUT20_SYMBOLS. "
            "Pass through the project's canonical universe before running."
        )
    ...
    for fold in folds:
        ...
        # Same chained capital for baseline and all treatment arms.
        if fold.fold == 1:
        ...
        for threshold in args.thresholds:
            ...
            gate_final = float(
                gate_metrics.get("final_equity", starting_capital)
            )
