...
def stress_parity(
    ...
    """
    Scale only execution costs:
      - commission
      - sell tax
      - slippage
    Strategy signals, ATR exits, position limits and exposure policy remain fixed.
    """
    return replace(
        parity,
        commission_rate=parity.commission_rate * multiplier,
        sell_tax_rate=parity.sell_tax_rate * multiplier,
        slippage_bps=parity.slippage_bps * multiplier,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "HOLDOUT20 cost/slippage stress: fixed Q70 + "
            "FRAGILE_BULL 30% exposure."
        )
    )
    parser.add_argument(
        "--market-health",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.30,
        help="FRAGILE_BULL exposure multiplier (default: 0.30)",
    )
    parser.add_argument(
        "--output",
        default="research_results/q70_cost_stress_holdout20",
    )
    args = parser.parse_args()

    if not 0.0 <= args.fragile_scale <= 1.0:
        raise ValueError("--fragile-scale must be between 0 and 1")
    ...
    print(
        "Experiment: Q70 fixed; Fragile exposure = "
        + str(args.fragile_scale)
        + "; cost multipliers = 1.0x, 1.5x, 2.0x"
    )
    ...
    for fold in folds:
        ...
        for cost_multiplier in (1.0, 1.5, 2.0):
            # Same Q70 candidate set and same FRAGILE 30% exposure.
            # Only transaction costs/slippage are stressed.
            candidates = copy.deepcopy(q70_candidates)

            stressed_parity = stress_parity(
                parity,
                cost_multiplier,
            )

            base_sizer = stressed_parity.build_position_sizer()

            sizer = Q70ExposureSizer(
                base_sizer=base_sizer,
                quality_model=quality,
                market_health=market_health,
                fragile_scale=args.fragile_scale,
            )

            simulator = make_simulator(
                parity=stressed_parity,
            ...
            equity_col = (
                ...
                else ("portfolio_value" if "portfolio_value" in equity.columns else equity.columns[-1])
            )
            eq = pd.to_numeric(equity[equity_col], errors="coerce").dropna()
            if len(eq):
                running_peak = eq.cummax()
                actual_dd_pct = float(((eq / running_peak) - 1.0).min() * 100.0)
            else:
                actual_dd_pct = 0.0
            ...
            fold_rows.append(
                {
                    "policy": (
                        f"q70_f{args.fragile_scale:g}_c{cost_multiplier:g}"
                    ),
                    "fold": fold.fold,
                    ...
                    "actual_max_drawdown_pct": actual_dd_pct,
            ...
            for t in executed:
                ...
                trade_rows.append(
                    {
                        "policy": f"q70_f{args.fragile_scale:g}_c{cost_multiplier:g}",
                        "fold": fold.fold,
                        "symbol": t.symbol,
    ...
    for name, g in folds_df.groupby(
        ...
        actual_dd = float(g["actual_max_drawdown_pct"].min()) if "actual_max_drawdown_pct" in g else np.nan
    ...
    print("\n=== Q70 + FRAGILE 30% HOLDOUT20 COST STRESS ===")
    print(
