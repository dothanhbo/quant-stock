...
def health(path):
    ...
    if missing:
        raise ValueError(f"market-health missing columns: {missing}; found: {d.columns.tolist()}")
...
def main():
 ...
 p.add_argument('--market-health',required=True)
 p.add_argument('--db-path',required=True)
 p.add_argument('--out',default='research_results/exposure_policy_wfo')
 p.add_argument('--start-date',default='2018-08-07')
 p.add_argument('--end-date',default='2026-08-21')
 p.add_argument('--thresholds',nargs='+',type=float,default=[0.65,0.70])
 p.add_argument('--fragile-scales',nargs='+',type=float,default=[0.50,0.25])
 a=p.parse_args()
 ...
 policies=[('baseline',None,None)]
 for threshold in a.thresholds:
  policies.append((f'q{int(round(threshold*100))}',threshold,1.0))
  for scale in a.fragile_scales:
   tag=str(scale).replace('.','')
   policies.append((f'q{int(round(threshold*100))}_f{tag}',threshold,scale))
