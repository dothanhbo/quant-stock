...
DEFAULT_OUTPUT_DIR = Path(
    "research_results/ablation_matrix_holdout20"
)
...
def build_cases() -> list[AblationCase]:
    return [
        AblationCase(
            case_id=f"{entry}__{exit_name}__{sizing}",
            entry=entry,
            exit=exit_name,
            sizing=sizing,
        )
        for entry in ("trend", "hybrid")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="2x2x2 controlled walk-forward ablation matrix."
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional case_id list. Omit to run all eight cases.",
    )
    return parser


def build_entry_model(name: str):
    if name == "trend":
...
def build_case_kwargs(
    *,
    ...
    return {
        ...
        "entry_model": build_entry_model(case.entry),
...
def main() -> None:
    ...
    for index, case in enumerate(cases, start=1):
        print("\n" + "#" * 96)
        print(f"ABLATION CASE {index}/{len(cases)}: {case.case_id}")
        print("#" * 96)
        ...
        folds.insert(0, "entry", case.entry)
    ...
    folds_path = output_dir / "matrix_folds.csv"
    summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    folds_df.to_csv(folds_path, index=False, encoding="utf-8-sig")

    print("\n" + "=" * 140)
    print("ABLATION MATRIX — ROBUSTNESS RANKING")
    print("=" * 140)
    columns = [
        "robust_rank",
        "case_id",
