...
DEFAULT_OUTPUT_DIR = Path(
    "research_results/entry_ablation_holdout20"
)


@dataclass(frozen=True, slots=True)
class EntryAblationCase:
    case_id: str
    use_regime_thresholds: bool
    require_hybrid_score: bool


def build_cases() -> list[EntryAblationCase]:
    return [
        EntryAblationCase(
            case_id="legacy_thresholds__no_hard_score",
            use_regime_thresholds=False,
            require_hybrid_score=False,
        ),
        EntryAblationCase(
            case_id="regime_thresholds__no_hard_score",
            use_regime_thresholds=True,
            require_hybrid_score=False,
        ),
        EntryAblationCase(
            case_id="legacy_thresholds__hard_score",
            use_regime_thresholds=False,
            require_hybrid_score=True,
        ),
        EntryAblationCase(
            case_id="regime_thresholds__hard_score__current",
            use_regime_thresholds=True,
            require_hybrid_score=True,
        ),
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled 2x2 entry ablation: Donchian regime "
            "thresholds x hybrid hard-score gate."
        )
    )
...
def compound_return(returns_pct: pd.Series) -> float:
    ...
    if values.empty:
        return 0.0
    return float(
        (np.prod(1.0 + values.to_numpy() / 100.0) - 1.0)
        * 100.0
    )
    ...
    *,
    ...
    return {
        ...
        "entry_model": HybridTrendDonchianEntryModel(
            mode="trend_context",
            use_regime_thresholds=case.use_regime_thresholds,
            require_hybrid_score=case.require_hybrid_score,
        ),
...
def main() -> None:
    ...
    for index, case in enumerate(cases, start=1):
        print("\n" + "#" * 96)
        print(f"ENTRY ABLATION {index}/{len(cases)}: {case.case_id}")
        print("#" * 96)
        ...
        pd.DataFrame(summary_rows).to_csv(
            output_dir / "entry_ablation_summary_partial.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    summary_path = output_dir / "entry_ablation_summary.csv"
    folds_path = output_dir / "entry_ablation_folds.csv"
    summary_df.to_csv(summary_path, index=False, encoding="utf-8-sig")
    folds_df.to_csv(folds_path, index=False, encoding="utf-8-sig")

    print("\n" + "=" * 140)
    print("ENTRY ABLATION — ROBUSTNESS RANKING")
    print("=" * 140)
    columns = [
        "robust_rank",
        "case_id",
