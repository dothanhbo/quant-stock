...
Runs five production-parity WFO variants using the existing
research.run_regime_policy_grid machinery:
1) baseline
2) hard reject: 3D return >= 6% AND volume ratio >= 2.5x
3) score penalty -5 on exhaustion candidates
4) score penalty -10 on exhaustion candidates
5) position size x0.5 on exhaustion candidates
...
class HalfSizeSizer:
    """Duck-typed PositionSizer wrapper: halve exhaustion quantities."""

    name = "exhaustion_half_size"

    def __init__(self, base: PositionSizer, store: ExhaustionFeatureStore):
        self.base = base
        self.store = store
    ...
    def calculate_quantity(self, context) -> int:
        ...
        if lot <= 0:
            return max(quantity // 2, 0)

        return (quantity // 2 // lot) * lot
    ...
    *,
    ...
    class ExhaustionAwarePortfolioSimulator(BasePortfolioSimulator):
        def __init__(self, *args, **kwargs):
            ...
            if mode == "size_half" and position_sizer is not None:
                kwargs["position_sizer"] = HalfSizeSizer(
                    position_sizer,
                    store,
                )
        ...
        def _open_candidate(
            ...
            *,
            ...
            if mode == "hard_reject" and exhausted:
                self._reject_trade(
            ...
            if mode == "score_minus_5" and exhausted:
                original = float(candidate.signal_score)
                candidate.signal_score = original - 5.0

            elif mode == "score_minus_10" and exhausted:
                original = float(candidate.signal_score)
                candidate.signal_score = original - 10.0
